...
gplot(complex_land) + 
  ...
  geom_text(aes(label = value), size = 4, color = "gray70") +
  coord_equal() +
...
The results, unsurprisingly, showed that the complex landscape has a larger value of the Boltzman entropy than the simple one.
...
The relative configurational entropy is a sum of entropies for all windows.
...
gplot(complex_land_l1) + 
  ...
  geom_text(aes(label = value), size = 4, color = "gray70") +
  coord_equal() +
  scale_fill_viridis_c(limits = c(12, 98)) +
...
We can explore this topic using five different cases of 2 x 2 windows:
...
The configurational entropy for data without missing values is calculated using the analytical method by Gao (2018).
